...
>
  ...
  <div class="tabs">
    <button
      ...
      class={graveyardTab == 'minions' ? 'active' : ''}>Minions</button
    >
    <button
      on:click={graveyardTabSelect('trophies')}
      class={graveyardTab == 'trophies' ? 'active' : ''}>Trophies</button
    >
  </div>
  <div
    class="upgrades"
    ng-if="graveyardTab == 'minions'"
  >
    <div class="bone-collectors bones">
      <h4>
        Bone Collectors <button
          on:click={(bcinfo = !bcinfo)}
          class={bcinfo ? 'active' : ''}>Info</button
        >
      </h4>
      <p ng-show="bcinfo">
        Hire bone collectors to gather bones from the town's dead and your own fallen zombies. Each
        bone collector consumes 1 energy per second. They're hungry little creatures. Must be all
        that running around.
      </p>
      <h4>Energy rate {decimal(model.getEnergyRate())} per second</h4>
      <button on:click={subtractBoneCollector()}>-</button><label
        >{whole(model.persistentData.boneCollectors)} bone collectors</label
    ...
    <div class="bone-collectors bones">
      <h4>
        Zombies to Spawn <button
          ...
          class={zsinfo ? 'active' : ''}>Info</button
        >
      </h4>
      <p ng-show="zsinfo">
        Control how many zombies the graveyard spawns each time your energy is full. This is limited
        by your maximum energy.
      </p>
      <div class="clear">
        ...
        <button on:click={setGraveyardZombies(model.persistentData.graveyardZombies + 1)}>+</button>
        <button on:click={setGraveyardZombies(maxGraveyardZombies())}
          >{maxGraveyardZombies()}</button
        >
      </div>
    </div>
    <div
      class="bone-collectors bones"
      ng-if="model.zombieCages > 0"
    >
      <h4>
        Caged Zombies <button
          on:click={(czinfo = !czinfo)}
          class={czinfo ? 'active' : ''}>Info</button
        >
      </h4>
      <div class="clear cages">
        <h4>You currently have {model.zombiesInCages} / {model.zombieCages} zombies caged</h4>
        <p ng-show="czinfo">
          You can release them to fight again, or sacrifice them to gain {whole(
            model.cagedZombieSacrificeValue().blood
          )} blood, {model.cagedZombieSacrificeValue().brains} brains, and {model.cagedZombieSacrificeValue()
            .bones} bones
        </p>
        <button
          on:click={model.releaseCagedZombies()}
          disabled={model.zombiesInCages == 0 || model.currentState != model.states.playingLevel}
          >Release</button
        >
        <button
          on:click={(model.persistentData.autoRelease = !model.persistentData.autoRelease)}
          class={model.persistentData.autoRelease ? 'active' : ''}>Auto Release</button
        >
        <button
    ...
    <div
      class="bone-collectors bones"
      ng-if="model.constructions.aviary"
    >
      <h4>
        Harpies <button
          on:click={(hpinfo = !hpinfo)}
          class={hpinfo ? 'active' : ''}>Info</button
        >
      </h4>
      <p ng-show="hpinfo">
        Release harpies that drop barrels of plague infected zombie flesh on unsuspecting humans.
        Each harpy consumes 1 energy per second. Harpy bombs infect humans with plague and cause {whole(
          model.zombieHealth * 0.2
        )} damage. The damage scales with zombie health.
      </p>
      <h4>Energy rate {decimal(model.getEnergyRate())} per second</h4>
  ...
  <p ng-if="graveyardTab == 'trophies'">
    Earn bonuses by collecting the severed heads of VIPs from certain towns. These trophies persist
    through prestige resets, while escaped VIPs will return to town.
  </p>
  <div
    class="tabs"
    ng-if="graveyardTab == 'trophies'"
  >
    <button
  ...
  >
    <div
      ng-repeat="trophy in trophies"
      class="upgrade bones {trophy.owned ? 'owned' : 'uncollected'}"
    >
      <label ng-if="trophyTab != 'totals'"
        >Level: {trophy.level}<span
          ng-if="trophy.escaped"
          class="escaped">VIP Escaped</span
        ></label
      >
