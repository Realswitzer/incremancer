  ...
  initialize() {
    ...
    if (this.sprites.length < this.maxParts) {
      for (var i = 0; i < this.maxParts; i++) {
        ...
        sprite.anchor = { x: 0.5, y: 0.5 }; // TODO: plsfix; syntax incorrect from origin src
...
const Bones = {
  ...
  newBones(x, y) {
    if (!GameModel.constructions.graveyard) return;
    for (var i = 0; i < this.partsPerSplatter; i++) {
      this.newPart(x, y);
    }
  },
...
const Blasts = {
  ...
  newBlast: function (x, y) {
    ...
    var sprite = this.sprites[this.partCounter++];
    if (this.partCounter >= this.maxParts) this.partCounter = 0;
