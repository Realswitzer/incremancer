...
export class Particles {
  ...
  constructor() {
    if (Particles.instance) return Particles.instance;
...
export class PrestigePoints extends SpritePool<GameObject> {
  ...
  constructor() {
    super();
    if (PrestigePoints.instance) return PrestigePoints.instance;
...
export class Blood {
  ...
  constructor() {
    if (Blood.instance) return Blood.instance;
    Blood.instance = this;
  }
  maxParts = 500;
  partCounter = 0;
  ...
  initialize(): void {
    ...
    if (this.sprites.length < this.maxParts) {
      for (let i = 0; i < this.maxParts; i++) {
        ...
        sprite.visible = false;
        if (Math.random() > 0.5) sprite.scale.set(2, 2);
...
export class Bones {
  ...
  constructor() {
    if (Bones.instance) return Bones.instance;
    Bones.instance = this;
  }
  ...
  updatePart(sprite: Bone, timeDiff: number): void {
    ...
    if (sprite.hitFloor) {
      if (this.fadeBones) sprite.fadeTime -= timeDiff;
...
export class Exclamations {
  ...
  constructor() {
    if (Exclamations.instance) return Exclamations.instance;
...
export class Bullets {
  ...
  constructor() {
    if (Bullets.instance) return Bullets.instance;
        ...
        if (
          ...
          if (sprite.rocket) {
            if (sprite.target.graveyard)
              this.graveyard.damageGraveyard(sprite.damage);
...
export class Blasts extends SpritePool<GameObject> {
  ...
  constructor() {
    super();
    if (Blasts.instance) return Blasts.instance;
...
export class Smoke extends SpritePool<GameObject> {
  ...
  constructor() {
    super();
    if (Smoke.instance) return Smoke.instance;
    Smoke.instance = this;
    this.create = (texture) => new GameObject(texture);
  }
  tint = 0xffffff;
...
export class Fragments extends SpritePool<Fragment> {
  ...
  constructor() {
    super();
    if (Fragments.instance) return Fragments.instance;
