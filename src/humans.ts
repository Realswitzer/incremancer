const Humans = {
  ...
  getMaxDoctors() {
    ...
    if (GameModel.level < 18) return 0;

    return maxDoctors;
  },

  getTorchChance() {
    if (GameModel.level < 10) return 0;
  ...
  update(timeDiff) {
    ...
    for (var i = 0; i < this.humans.length; i++) {
      ...
      if (!this.humans[i].dead) aliveHumans.push(this.humans[i]);
    }
  ...
  updateHuman(human, timeDiff, aliveZombies) {
    if (human.dead) return this.updateDeadHumanFading(human, timeDiff);

    human.attackTimer -= timeDiff;
    human.timeToScan -= timeDiff;
    human.timeFleeing -= timeDiff;

    if (human.infected) this.updatePlague(human, timeDiff);
...
const Police = {
  ...
  decideStateOnZombieDistance(police) {
    if (police.zombieTarget && !police.zombieTarget.dead) {
      ...
      var distanceToTarget = fastDistance(
        police.position.x,
        police.position.y,
  ...
  updatePolice(police, timeDiff, aliveZombies) {
    if (police.dead) return Humans.updateDeadHumanFading(police, timeDiff);
  ...
  updatePoliceDog(dog, timeDiff, aliveZombies) {
    ...
    if (dog.infected) Humans.updatePlague(dog, timeDiff);
    if (dog.burning) Humans.updateBurns(dog, timeDiff);
...
const Army = {
  ...
  populate() {
    ...
    for (var i = 0; i < maxArmy; i++) {
      ...
      if (
        (GameModel.level > 45 && Math.random() < 0.3) ||
        (GameModel.isBossStage(GameModel.level) && Math.random() < 0.5)
      ) {
        textureId = 2;
      }
  ...
  updateArmy(armyman, timeDiff, aliveZombies) {
    if (armyman.dead) return Humans.updateDeadHumanFading(armyman, timeDiff);
      ...
      case this.states.shooting:
        if (armyman.zombieTarget && !armyman.zombieTarget.dead) {
          ...
          if (armyman.attackTimer < 0) {
            ...
            armyman.attackTimer = armyman.rocketlauncher
...
const Tanks = {
  ...
  updateTank(tank, timeDiff, aliveZombies) {
    if (tank.dead) return Humans.updateDeadHumanFading(tank, timeDiff);

    tank.attackTimer -= timeDiff;
    tank.timeToScan -= timeDiff;

    if (tank.burning) Humans.updateBurns(tank, timeDiff);
...
export { Humans, Police, Army, Tanks };
