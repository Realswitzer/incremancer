...
export class Controller {
    ...
    toggleAutoStart = function () {
      if (model.persistentData.autoStart) {
        model.persistentData.autoStart = false;
      } else {
        model.persistentData.autoStart = true;
      }
    };

    toggleResolution = function (resolution) {
      model.persistentData.resolution = resolution;
      model.setResolution(model.persistentData.resolution);
    };

    getResolution = function () {
      return model.persistentData.resolution || 1;
    };

    toggleZoomButtons = function () {
      model.persistentData.zoomButtons = !model.persistentData.zoomButtons;
    };
    ...
    resetZoom = function () {
      model.centerGameContainer(true);
    };

    toggleShowFps = function () {
      model.persistentData.showfps = !model.persistentData.showfps;
    };

    toggleParticles = function () {
      model.persistentData.particles = !model.persistentData.particles;
    };

    isShowPrestige = function () {
      if (typeof model.persistentData.prestigePointsEarned === 'undefined') return false;
      return model.persistentData.allTimeHighestLevel > 5;
    };
    ...
    constructionLeadsTo = function (upgrade) {
    ...
    updateMessages = function (timeDiff) {
      if (message) {
        messageTimer -= timeDiff;
        if (model.messageQueue.length > 0) messageTimer -= timeDiff;
        if (messageTimer < 0) {
          message = false;
          messageTimer = 4;
        }
      } else {
        if (model.messageQueue.length > 0) {
          message = model.messageQueue.shift();
          messageTimer = 4;
        }
      }
    };
    ...
    infuseRune = function (rune, cost) {
      if (infusionMax) {
        switch (cost) {
          case 'blood':
            Upgrades.infuseRune(rune, cost, model.persistentData.blood);
            break;
          case 'brains':
            Upgrades.infuseRune(rune, cost, model.persistentData.brains);
            break;
          case 'bones':
            Upgrades.infuseRune(rune, cost, model.persistentData.bones);
            break;
        }
      } else {
        Upgrades.infuseRune(rune, cost, infusionAmount);
      }
    };

    shatterPercent = function (rune) {
      return Upgrades.shatterPercent(rune);
    };

    shatterBloodCost = function (rune) {
      return Upgrades.shatterBloodCost(rune);
    };

    shatterSatiate = function (runetype, rune) {
    ...
    shatterEffect = function () {
      return Upgrades.shatterEffect() * 100;
    };

    infuseButtonText = function () {
      if (infusionMax) {
        return 'Max';
      } else {
        return formatWhole(infusionAmount);
      }
    };

    energyPercent = function () {
      return Math.min(Math.round((model.energy / model.energyMax) * 100), 100);
    };
    bloodPercent = function () {
    ...
    costAboveCap = function (upgrade, price) {
      switch (upgrade.costType) {
        case 'blood':
          if (price > model.bloodMax) {
            return 'Blood capacity too low';
          }
          break;
        case 'brains':
          if (price > model.brainsMax) {
    ...
    upgradeButtonText = function (upgrade) {
      if (upgrade.cap != 0 && currentRank(upgrade) >= upgrade.cap) return 'Sold Out';

      var price = upgradePrice(upgrade);

      if (upgradeTooExpensive(upgrade)) {
        var aboveCap = costAboveCap(upgrade, price);
        if (aboveCap) return aboveCap;
        return requiredForUpgrade(upgrade);
      }

      return purchaseText(upgrade, price);
    };
    ...
    function innerUpdate(timeDiff, updateTime) {
      model.update(timeDiff, updateTime);
      updateMessages(timeDiff);
    }
