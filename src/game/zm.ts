  ...
  switch (type) {
    ...
    case 'prestige':
      ...
      zm.upgrades.push.apply(
        zm.upgrades,
        Upgrades.prestigeUpgrades.filter((upgrade) => upgrade.cap !== 0 && zm.currentRank(upgrade) >= upgrade.cap)
      );
...
zm.setGraveyardZombies = function (number) {
  if (number <= zm.maxGraveyardZombies() && number >= 0) zm.model.persistentData.graveyardZombies = number;
};
